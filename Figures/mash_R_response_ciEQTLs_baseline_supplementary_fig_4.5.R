...
source("~/bin/Rscripts/ggplot_themes.R")

# supplementary figures 4-5

ci_eQTLs<-read.table("~/myscratch/MacroMap/Data/jr35/macromap_np_ci_eqtl/shared_to_nikos/all_sentinel_ci_eqtls.txt",header=TRUE)

ci_eQTLs %>% group_by(condition,rank,phe_id) %>% filter (rank >0) %>% distinct(pos=paste0(phe_chr,":",phe_from,"-",phe_to),rs=var_id,rs_pos=paste0(phe_chr,":",var_from,"-",var_to))
ci_eQTLs_cond<-ci_eQTLs %>% group_by(condition,rank,phe_id) %>% filter (rank >0) %>% distinct(pos=paste0(phe_chr,":",phe_from,"-",phe_to),rs=var_id,eqtl=paste0(phe_id,"_",var_id,"_",phe_chr,"_",var_from))
...
p<-ggplot(data=toplot,aes(y =n, x = cie_per_gn)) + 
  ...
  geom_text(aes(label=n), colour = "black", size = 5, vjust=-0.5, hjust = 0.5) +
  labs(x = "Number of independent cis-eQTLs per gene", y = "Number of eGenes") + theme_Publication(20) + 
  annotate("text", x = 3, y = 2600, label = "18% of eGenes have >1 CI cis-eQTL", size = 5.5, fontface = "bold") 

pdf("CI_eQTLs_barplot.pdf",useDingbats = FALSE)
print(p)
dev.off()
...
pdf("CI_eQTLs_tss_distance.pdf",useDingbats = FALSE,width =7 ,height = 7)
print(p1)
dev.off()

pdf("CI_eQTLs_barplot_CI_eQTLs_tss_distance.pdf",useDingbats = FALSE,width =14 ,height = 7)
grid.arrange(p,p1,ncol=2, widths = c(7, 7))
dev.off()

#########################################################################################################################################################
#load the data 
qtls<-readRDS("~/myscratch/MacroMap/Analysis/eQTLs/Macromap_fds/analysis/conditional_analysis_jesse/q.RDS")
path_mashR<-"~/myscratch/MacroMap/Analysis/eQTLs/Macromap_fds/analysis/mashR/analysis/analysisV2/8_flashR_best_eQTLs_random200K_commonbaseline/"

m.ed_common_baseline<-readRDS(paste0(path_mashR,"m.ed.RDS"))
Vhat<-readRDS(paste0(path_mashR,"Vhat.RDS"))
...
############################################################################################################################################################################################################
sign_betas<-readRDS("sign_betas.RDS")
sign_std<-readRDS("sign_std.RDS")
...
saveRDS(m.ed.t_common_baseline,file="~/myscratch/MacroMap/Analysis/eQTLs/Macromap_fds/analysis/conditional_analysis_jesse/m.ed.t_baseline_secondary_eQTLs.RDS")
...
# 
# 
# # do this once and save the pbject 
...
# }
# 
# saveRDS("~/myscratch/MacroMap/Analysis/eQTLs/Macromap_fds/analysis/conditional_analysis_jesse/q.RDS")

### DONE ! 
m.ed.t_common_baseline<-readRDS("m.ed.t_baseline_secondary_eQTLs.RDS")
...
pdf(paste0("barplots_response_eQTLs_condition_by_condition.pdf"))
par(mar = c(5,7, 3,2))
...
# 
# dat <- data.frame(
#   "Response eQTLs" = c(263, 2374),
#   "Shared eQTLs" = c(1816-263, 10170-2374),
#   row.names = c("Non Primary eQTLs", "Primary eQTLs"),
#   stringsAsFactors = FALSE
# )
# colnames(dat) <- c("Response eQTLs", "Shared eQTLs")
# 
...
# }
# df <- as.data.frame(x)
# colnames(df) <- c("Order_of_eQTL", "Type_of_eQTL")
# 
...
# library(ggstatsplot)
# p<-ggbarstats(
#   df, Type_of_eQTL, Order_of_eQTL,
#   results.subtitle = FALSE,
#   subtitle = paste0(
#     "Fisher's exact test", ", p-value = ",
#     ifelse(test$p.value < 0.001, round(test$p.value, 18), round(test$p.value, 18))
#   )
# )
# 
# p<-p +theme_Publication(20) + scale_fill_brewer(palette="Accent")
# 
# pdf("percentages_shared_response_primary_secondary.pdf")
# print(p)
# dev.off()
#######################################################################################################################################
...
path_to_read_files<-"~/myscratch/MacroMap/Analysis/eQTLs/Macromap_fds/analysis/mashR/analysis/8_flashR_condition_by_condition/8_flashR_condition_by_condition_baseline/"
response_QTLs<-readRDS(file=paste0(path_to_read_files,"response_per_conditions.RDS"))

path_to_read_files<-"/nfs/users/nfs_n/np12/myscratch/MacroMap/Analysis/eQTLs/Macromap_fds/analysis/mashR/files/8_flashR_condition_by_condition/"
response_QTLs_fake<-readRDS(file=paste0(path_to_read_files,"response_per_conditions.RDS"))

names(response_QTLs)<-names(response_QTLs_fake)[-c(3,4)]
names(response_QTLs)<-sapply(strsplit(unlist(strsplit(names(response_QTLs),"_significant")),"condition_"),"[[",2)
...
dat <- data.frame(
  "Primary eQTL is reQTL" = c(no_sec_reQTLs_primary_res, sec_reQTLs_primary_res),
  "Primary eQTL is not reQTL" = c(no_sec_reQTLs_primary_nores, sec_reQTLs_primary_nores),
  row.names = c("CI-eQTLs that are not reQTLs","CI-eQTLs that are reQTLs"),
  stringsAsFactors = FALSE
)
colnames(dat) <- c("Primary eQTL is reQTL", "Primary eQTL is not reQTL")
...
df <- as.data.frame(x)
colnames(df) <- c("Conditionally independent eQTLs", "Primary eQTL")
...
library(ggstatsplot)
p<-ggbarstats(
  df, "Primary eQTL", "Conditionally independent eQTLs",
  results.subtitle = FALSE,
  subtitle = paste0(
    "Fisher's exact test", ", p-value = ",
    ifelse(test$p.value < 0.001, round(test$p.value, 36), round(test$p.value, 36))
  )
)

source("~/bin/Rscripts/ggplot_themes.R")
p<-p +theme_Publication(16) + scale_fill_brewer(palette="Dark2")


pdf("percentages_shared_response_primary_secondary.pdf",useDingbats = FALSE)
print(p)
dev.off()
